...
public class VoxelWorld_TestScene_4_2_2 extends SceneBase {
	...
	public static class Factory implements Function1<Scene, Double> {
		...
		@Override public Scene at(Double time) {
			try {
				return new VoxelWorld_TestScene_4_2_2(
						seed, cameraAngle,
	...
	public VoxelWorld_TestScene_4_2_2(
			...
			for (int j = 0; j < dim.yInt(); j++) {
				for (int k = 0; k < dim.zInt(); k++) {
					...
					if (rng.nextDouble() < prob) {
						arr0[i][j][k] = true;
						arr1[i][j][k] = Color.rgb(rng.nextDouble(), .25, rng.nextDouble());
					}
				}
			}
		}
