...
public class CityOfLight extends SceneBase {
	...
	public static class Factory implements Function1<Scene, Double> {
		...
		public Scene at(Double time) {
			return new CityOfLight(n, seed);
		}
	}
	
	
	public CityOfLight(int n, long seed) {
		...
		while (boxes.size() < n && nTrials < 10000) {
			nTrials++;
			Vector c = sampler.randomInBox(mars.geometry.Box.cr(3));
			...
			if (boxes.stream().noneMatch(b -> b.intersects(bCandidate))) {
				...
				Solid solid = Box.$.pd(Vec3.py(bCandidate.p(), 0), Vec3.py(bCandidate.d(), h));
