...
public interface BSDF {
	...
	class Result {
		...
		public Result(Vec3 out, Color color) {
			this.out = out;
			this.color = color;
		}
	...
	static BSDF add(BSDF[] bsdfs, double[] weights) {
		...
		if (m == 2) return mix(bsdfs_[0], bsdfs_[1], weights_[0]);
	...
	BSDF REFLECTIVE = (sampler, n_, i) -> new Result(Utils.reflected(n_, i), Color.WHITE);
	
	BSDF TRANSMISIVE = transmissive(Color.WHITE);
	...
	static BSDF diffuse(Color c) {
		return (sampler, n_, i) -> new Result(Utils.sampleHemisphereCosineDistributed(sampler, n_), c);
...
//		};
	...
	static BSDF reflective(Color c) {
		return (sampler, n_, i) -> new Result(Utils.reflected(n_, i), c);
	}
...
*/
	...
	static BSDF glossyRefractive(Color c, double refractiveIndex, double s) {
		return (sampler, n_, i) -> {
			Vec3 r = Utils.refracted(refractiveIndex, n_, i.normalized_());
			Vec3 d = Utils.sampleHemisphereCosineDistributed(sampler, n_).mul(Numeric.sign(n_.dot(r)));
	...
	static BSDF refractive(Color c, double refractiveIndex) {
		...
		return (sampler, n_, i) -> new Result(Utils.refracted(refractiveIndex, n_, i), c);
	}
